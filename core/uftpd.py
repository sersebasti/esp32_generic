...
class FTP_client:
    ...
    def exec_ftp_command(self, cl):
        ...
        try:
            ...
            # Enforce login when credentials configured
            ...
            if (_AUTH_USER is not None) and (not self.logged_in) and (command not in ("USER", "PASS", "QUIT")):
                cl.sendall("530 Not logged in.\r\n")
                client_busy = False
                return

            if command == "USER":
                self._user = payload.strip() if payload else None
                ...
                if _AUTH_USER is not None:
                    ok_user = (self._user == _AUTH_USER)
                    ok_pass = ((payload or "").strip() == (_AUTH_PASS or ""))
