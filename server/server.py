import socket, gc, ujson, network, sys
from server.http_consts import (_HTTP_200_JSON, _HTTP_200_HTML, _HTTP_400, _HTTP_204_CORS)
from server.busy_lock import set_busy
from server import status_api, system_api
...
def _get_adc_api():
    global adc_api, adc_api_err
    if adc_api is not None:
        return adc_api
    if adc_api_err is not None:
        return None
    try:
        cached = sys.modules.get("scope.adc_api")
...
def start_server(preferred_port=80, fallback_port=8080, verbose=True):
    ...
    try:
        ...
        s.settimeout(0.5)
        if verbose:
            # Avoid initializing WLAN here to reduce driver init pressure on boot.
        ...
        while True:
            ...
            try:
                ...
                if path.startswith("/fs/"):
                    if (fs_api is None) or (not feature_enabled("fs")):
                        cl.send(_HTTP_400)
                        if verbose:
                ...
                if status_api.handle(cl, method, path):
                    ...
                    feature_enabled("wifi")
                    and wifi_api is not None
                    and wifi_api.handle(cl, method, path, req, _read_post_json)
                ):
                    result = "wifi_api"
                elif (
                    feature_enabled("scope")
                    and _is_scope_path(path)
                    and (_get_adc_api() is not None)
                    and adc_api.handle(cl, method, path, req, _read_post_json)
                ):
                    result = "adc_api"
                elif feature_enabled("scope") and (_get_adc_api() is None) and _is_scope_path(path):
                    cl.send(_HTTP_400)
                    result = "scope_import_failed"
                    if verbose:
                        try: print("[HTTP] 400 scope: adc_api import failed: %r" % (adc_api_err,))
                        except Exception: pass
                elif (
                    ...
                    and feature_enabled("my_webrepl")
                ...
                if verbose:
                    try: print("[HTTP] result=%s" % (result,))
                    except Exception: pass
            except Exception as e:
                err_no = None
                try:
